...
Unit Price: ' . $row['unit_price'] . '<br>
        ...
        try {
            ...
            $mailer->addAddress($cust_email); // Customer's email

            // Email content
            $mailer->isHTML(true);
            $mailer->Subject = $subject_text;
            $mailer->Body = '
            <html>
            <body>
                <h3>Message:</h3>
            ...
            $error_message = 'An error occurred while sending the email. Error: ' . $mailer->ErrorInfo;
        }
    }
}
?>
                                ...
                                <tr class="<?php if ($row['payment_status'] == 'Pending') {
                                    ...
                                    <td>
                                        ...
                                        foreach ($result1 as $row1) {
                                            ...
                                            echo '<br>(<b>SPH:</b> ' . $row1['size'];
                                            echo ', <b>CYL:</b> ' . $row1['color'] . ')';
                                    ...
                                    <td>
                                        ...
                                        <a href="generate-invoice.php?payment_id=<?php echo $row['payment_id']; ?>" class="btn btn-primary btn-xs" style="width:100%;margin-bottom:4px;" target="_blank">Generate Invoice</a>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        // Retrieve the uploaded invoice for the current payment_id
                                        $statement2 = $pdo->prepare("SELECT upload_invoice FROM tbl_order WHERE payment_id=?");
                                        $statement2->execute(array($row['payment_id']));
                                        $result2 = $statement2->fetch(PDO::FETCH_ASSOC);
                                        $uploaded_invoice = $result2['upload_invoice'];
                                        ?>

                                        <?php if (!empty($uploaded_invoice)) : ?>
                                            <a href="uploads/<?php echo $uploaded_invoice; ?>" class="btn btn-info btn-xs" style="width:100%;margin-bottom:4px;" target="_blank">View Invoice</a>
                                        <?php else : ?>
