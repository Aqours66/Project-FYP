...
if (isset($_POST['form1'])) {
	...
	if (empty($_POST['supplier_name'])) {
		$valid = 0;
		$error_message .= "You must have to select a supplier<br>";
	}
	...
	if ($valid == 1) {
		...
		if (isset($_POST['Add'])) {
			...
			$statement = $pdo->prepare("DELETE FROM tbl_product_Add WHERE p_id=?");
			$statement->execute(array($_REQUEST['id']));

			foreach ($_POST['Add'] as $value) {
				$statement = $pdo->prepare("INSERT INTO tbl_product_Add (Add_id,p_id) VALUES (?,?)");
				$statement->execute(array($value, $_REQUEST['id']));
			}
		} else {
			$statement = $pdo->prepare("DELETE FROM tbl_product_Add WHERE p_id=?");
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Supplier <span>*</span></label>
							<div class="col-sm-4">
								<select name="supplier_name" class="form-control select2">
									<option value="">Select Supplier</option>
									<?php
									$statement = $pdo->prepare("SELECT * FROM tbl_supplier ORDER BY supplier_name ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);
									foreach ($result as $row) {
									?>
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select SPH</label>
							<div class="col-sm-4">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select CYLr</label>
							<div class="col-sm-4">
						...
						<div class="form-group">
							<label for="" class="col-sm-3 control-label">Select Add</label>
							<div class="col-sm-4">
								<select name="Add[]" class="form-control select2" multiple="multiple">
									<?php
									$is_select = '';
									$statement = $pdo->prepare("SELECT * FROM tbl_Add ORDER BY Add_id ASC");
									$statement->execute();
									$result = $statement->fetchAll(PDO::FETCH_ASSOC);
									foreach ($result as $row) {
										if (isset($Add_id)) {
											if (in_array($row['Add_id'], $Add_id)) {
												$is_select = 'selected';
											} else {
												$is_select = '';
											}
									...
									?>
										<option value="<?php echo $row['Add_id']; ?>" <?php echo $is_select; ?>><?php echo $row['Add_name']; ?></option>
									<?php
									}
									?>
								</select>
