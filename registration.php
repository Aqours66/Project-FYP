...
if (isset($_POST['form1'])) {
    ...
    try {
        ...
        $mail->Password = 'shkclnxfqzhnxsgg';              // SMTP password
        $mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
        $mail->Port = 587;                                    // TCP port to connect to

        //Recipients
        $mail->setFrom('ongweian1234@gmail.com', 'Lensify');
        $mail->addAddress($_POST['cust_email'], $_POST['cust_name']);     // Add a recipient

        //Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = 'Account Verification';
        $mail->Body    = 'Please click the following link to activate your account: <a href="http://localhost/Lensify/verify.php?token=' . $token . '">Verify Account</a>';

        $mail->send();
        $success_message = 'A verification email has been sent to your email address. Please click on the link in the email to activate your account.';
    } catch (Exception $e) {
...
<div class="page-banner" style="background-color:#444;background-image: url(assets/uploads/<?php echo $banner_registration; ?>);">
    <div class="inner">
        ...
        <?php
        if ($error_message != '') {
            echo "<div class='error' style='padding: 10px;background:#f1f1f1;margin-bottom:20px;text-align:center;'>" . $error_message . "</div>";
        }
        if ($success_message != '') {
            echo "<div class='success' style='padding: 10px;background:#f1f1f1;margin-bottom:20px;text-align:center;'>" . $success_message . "</div>";
        }
        ?>
    </div>
</div>
