...
$statement = $pdo->prepare("SELECT * FROM tbl_product_add WHERE p_id=?");
$statement->execute(array($_REQUEST['id']));
$result = $statement->fetchAll(PDO::FETCH_ASSOC);
foreach ($result as $row) {
    $add[] = $row['Add_id'];
                                    ...
                                    <div class="row">
                                        ...
                                        <?php if (isset($add)) : ?>
                                            <div class="col-md-12" style="margin-top: 20px;">
                                                SELECT ADD <br>
                                                <select name="Add_id" class="form-control select2" style="width:auto;">
                                                    <?php
                                                    $statement = $pdo->prepare("SELECT * FROM tbl_add");
                                                    $statement->execute();
                                                    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                                    foreach ($result as $row) {
                                                        if (in_array($row['Add_id'], $add)) {
                                                    ?>
